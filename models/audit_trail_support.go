...
type AuditTrailSupport struct {
	...
	CompanyColumnName     *string          `bun:"company_column_name,type:varchar(255)"`                        // Used during purge, identifies company column name in source table
	LocationColumnName    *string          `bun:"location_column_name,type:varchar(255)"`                       // Used during purge, identifies location column name in source table
	CompletedColumnName   *string          `bun:"completed_column_name,type:varchar(255)"`                      // Used during purge, identifies completed column name in source table
	...
	DateCreated           *mssql.DateTime1 `bun:"date_created,type:datetime,default:(getdate())"`               // Date record is created
	CreatedBy             string           `bun:"created_by,type:varchar(255),default:(suser_sname())"`         // Identifies the person who created the record
	DateLastModified      mssql.DateTime1  `bun:"date_last_modified,type:datetime,default:(getdate())"`         // Last date the record was modified
