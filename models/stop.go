...
type Stop struct {
	...
	BeginStopDate      *mssql.DateTime1 `bun:"begin_stop_date,type:datetime"`        // Date time that the driver signed on at the stop location.
	EndStopDate        *mssql.DateTime1 `bun:"end_stop_date,type:datetime"`          // Date time that the driver signed off at the stop location.
