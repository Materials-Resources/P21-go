...
type Payments struct {
	...
	HomeChkAmount              *float64         `bun:"home_chk_amount,type:decimal(19,4)"`                              // Check amount in terms of the company currency ID
	VoidPeriod                 *float64         `bun:"void_period,type:decimal(3,0)"`                                   // What period was this payment voided in?
	VoidYear                   *float64         `bun:"void_year,type:decimal(4,0)"`                                     // What yar was this payment voided in?
	StatementClearedDate       *mssql.DateTime1 `bun:"statement_cleared_date,type:datetime"`                            // Date in which the check was cleared
	...
	TransmissionMethod         *int32           `bun:"transmission_method,type:int"`                                    // column is used to identify payments that have been sent using EDI
	Reason                     *string          `bun:"reason,type:varchar(255)"`                                        // to hold the reason for a voided check
	...
	CheckMailedDate            *mssql.DateTime1 `bun:"check_mailed_date,type:datetime"`                                 // Custom Feature 28424: Date check was mailed to vendor
	HomeCurrencyAmount         *float64         `bun:"home_currency_amount,type:decimal(19,9)"`                         // Custom column to store the total home currency amount for a vendor check
	CalculatedExchangeRate     *float64         `bun:"calculated_exchange_rate,type:decimal(19,9)"`                     // Custom exchange rate calculated using home currency amount and vendor check total
	...
	VoidDate                   *mssql.DateTime1 `bun:"void_date,type:datetime"`                                         // Date Check was Voided
	BankGlAcctNo               *string          `bun:"bank_gl_acct_no,type:varchar(255)"`                               // Bank GL acct debited at time of payment.
	ExchangeRate               *float64         `bun:"exchange_rate,type:decimal(19,6)"`                                // Stores the exchange rate used.
	SourceTypeCd               *int32           `bun:"source_type_cd,type:int"`                                         // Code (from code_p21 table) which indicates where the Check was created
}
