...
type InvLoc struct {
	...
	NextDueInPoDate                 *mssql.DateTime1 `bun:"next_due_in_po_date,type:datetime"`                         // Date that we expect a PO with this item to arrive.
	...
	LastSaleDate                    mssql.DateTime1  `bun:"last_sale_date,type:datetime,default:('01/01/1990')"`       // Last date order line is created for the item,location combination.
	LastPurchaseDate                *mssql.DateTime1 `bun:"last_purchase_date,type:datetime"`                          // Stores the date of the last Purchase Order for an item/location.  Po_line.date_created is used.
	...
	InvLastChangedDate              *mssql.DateTime1 `bun:"inv_last_changed_date,type:datetime"`                    // Indicates the date the item has had stock activity.
	...
	DemandPatternEvaluationDate     *mssql.DateTime1 `bun:"demand_pattern_evaluation_date,type:datetime"`           // The last time the demand pattern was evaluated by the system for this item/location.
	...
	EffectiveDate                   *mssql.DateTime1 `bun:"effective_date,type:datetime"`                           // This column indicates the effective date from which the future standard cost would be effective
