...
type OeLineSecondaryRebate struct {
	...
	SecondaryRebateTypeCd     *int32          `bun:"secondary_rebate_type_cd,type:int"`                               // The type of the secondary rebate (None, Source, Value).
	SecondaryRebateValue      *float64        `bun:"secondary_rebate_value,type:decimal(19,9)"`                       // The decimal value used for calculating the secondary rebate.
	...
	OtherCostSourceCd         *int32          `bun:"other_cost_source_cd,type:int"`                                   // Indicates the source for the other cost.
	CostCalculationValue      *float64        `bun:"cost_calculation_value,type:decimal(19,9)"`                       // Indicates the multiplier that would be used in conjunction with calculated other cost.
	OtherCostSourcePriceOe    *float64        `bun:"other_cost_source_price_oe,type:decimal(19,9)"`                   // Indicates the source price at OE for the other cost.
	SecondayRebateSrcPriceOe  *float64        `bun:"seconday_rebate_src_price_oe,type:decimal(19,9)"`                 // Indicates the source price at OE for the secondary rebate.
}
