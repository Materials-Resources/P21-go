...
type Users struct {
	...
	Password                          *string          `bun:"password,type:varchar(40)"`                                     // What is the user
	...
	DefaultCompany                    *string          `bun:"default_company,type:varchar(8)"`                               // What is the default company for this user?
	DefaultBranch                     *string          `bun:"default_branch,type:varchar(8)"`                                // In selected areas, branch id will default to this value.
	...
	CreateCustomers                   *string          `bun:"create_customers,type:char(1)"`                                 // Can this user create customers?
	CreateShipTos                     *string          `bun:"create_ship_tos,type:char(1)"`                                  // Can this user create Ship To data?
	DefaultLocationId                 *float64         `bun:"default_location_id,type:decimal(19,0)"`                        // In selected areas, location id will default to this value.
	PromptBeforeClearing              *string          `bun:"prompt_before_clearing,type:char(1)"`                           // Should this user be prompted before clearing?
	DefaultCustomerId                 *float64         `bun:"default_customer_id,type:decimal(19,0)"`                        // What is the default customer for this user?
	...
	CreateItemsAtOe                   *string          `bun:"create_items_at_oe,type:char(1)"`                               // Can this user create items while in Order Entry?
	DefaultApplication                *string          `bun:"default_application,type:varchar(40)"`                          // This is the module that will be opened after logging in.
	ScriptPath                        *string          `bun:"script_path,type:varchar(255)"`                                 // This is the destination folder for CommerceCenter script. Used primarily by Jetform.
	...
	ClassId                           *string          `bun:"class_id,type:varchar(8)"`                                      // Identifier for the class.
	...
	SalesrepId                        *string          `bun:"salesrep_id,type:varchar(16)"`                                  // If the user is a Sales Representative, this column stores the Sales Rep ID for the user.
	...
	TimeZoneCd                        *int32           `bun:"time_zone_cd,type:int"`                                         // Default time zone for the user
	...
	DefaultLabelPrinter               *string          `bun:"default_label_printer,type:varchar(255)"`                       // Label printer this user uses.
	...
	DefaultSalesrepOnOrder            *string          `bun:"default_salesrep_on_order,type:varchar(16)"`                    // This salesrep will be defaulted on orders taken by this user.
	...
	OrderValidationPassword           *string          `bun:"order_validation_password,type:varchar(255)"`                   // Password column to validate hold orders
	BypassCheckVerifyPassword         *string          `bun:"bypass_check_verify_password,type:varchar(255)"`                // Identifies a user who is able to bypass verifying a credit card payment
	MachineName                       *string          `bun:"machine_name,type:varchar(255)"`                                // The hostname() of the machine this user connects with
	...
	VacationEndDate                   *mssql.DateTime1 `bun:"vacation_end_date,type:datetime"`                               // The last day of a users scheduled vacation
	DoeSalesrepId                     *string          `bun:"doe_salesrep_id,type:varchar(16)"`                              // The salesrep_id that this user logs orders with in disconnected mode
	...
	UsersUid                          int32            `bun:"users_uid,type:int,autoincrement,identity"`                     // Unique identifier on the users table.
	InvGroupHdrUid                    *int32           `bun:"inv_group_hdr_uid,type:int"`                                    // Inventory mgmt group assigned to this user.
	...
	PoApprovalThreshold               *float64         `bun:"po_approval_threshold,type:decimal(19,9)"`                      // The dollar amount that limits the approval of POs
	...
	ActiveDirectoryRole               *string          `bun:"active_directory_role,type:varchar(255)"`                       // Role kept in Active Directory and updated when the user logs into P21
	UpdateBranchOeReportsFlag         *string          `bun:"update_branch_oe_reports_flag,type:char(1)"`                    // The column is to indicate whether user can update branch id for sales order report and wbw report.
	OeAllowShipmentEdits              *string          `bun:"oe_allow_shipment_edits,type:char(1)"`                          // The column is to indicate whether a user can edit a shipment in Shipping window after packing list has been printed
	OeAllowPackingListReprint         *string          `bun:"oe_allow_packing_list_reprint,type:char(1)"`                    // The column is to indicate whether a user can reprint a packing list in Shipping window after original packing list has been printed
	SalesrepSecurityType              *string          `bun:"salesrep_security_type,type:char(1)"`                           // Salesrep Security Restriction level
	UserSalesrepId                    *string          `bun:"user_salesrep_id,type:varchar(16)"`                             // Salesrep ID associated with user for security purposes
	...
	ConfirmDeaPtFlag                  *string          `bun:"confirm_dea_pt_flag,type:char(1)"`                              // Control whether or not a user can confirm DEA pick tickets
	StrategicPricingRoleUid           *int32           `bun:"strategic_pricing_role_uid,type:int"`                           // The strategic_pricing_role_uid for this users.
	CashDrawerId                      *string          `bun:"cash_drawer_id,type:varchar(8)"`                                // Custom column for the user to default a cash drawer.
	DfltWwmsFormsPrinter              *string          `bun:"dflt_wwms_forms_printer,type:varchar(255)"`                     // Custom (F40090): default WWMS forms printer - user's default printer to print unconfirmed packing lists
	...
	CustomerProfitRoleUid             *int32           `bun:"customer_profit_role_uid,type:int"`                             // Customer Profitability Dashboard Role
	AllowPostToClosedGlPeriod         *string          `bun:"allow_post_to_closed_gl_period,type:char(1)"`                   // A flag to determine whether a user can post to a closed GL period
	...
	DisplayOpenQuoteLinesCd           *int32           `bun:"display_open_quote_lines_cd,type:int"`                          // Allows the user to price their order lines from previous quotes in order entry.
	CreateItemsAtSoe                  *string          `bun:"create_items_at_soe,type:char(1)"`                              // Can this user create items while in Service Order Entry?
	...
	ShowCcPaymentAcctOnSave           *int32           `bun:"show_cc_payment_acct_on_save,type:int"`                         // Display Element Payment Account Response on th eselected areas
	ShowArCcFailed                    *string          `bun:"show_ar_cc_failed,type:char(1)"`                                // Display Process to A/R option when Credit Card process fails
	...
	LogoPathFilename                  *string          `bun:"logo_path_filename,type:varchar(255)"`                          // Logo path definable at the user level.
	UserReportPath                    *string          `bun:"user_report_path,type:varchar(255)"`                            // User level custom forms path.
	...
	ExtendedItemInfoPorgFlag          *string          `bun:"extended_item_info_porg_flag,type:char(1)"`                     // Indicates whether to show extended item information in PORG for a user.
	FedexLabelPrinterPath             *string          `bun:"fedex_label_printer_path,type:varchar(255)"`                    // Fedex Label printer for a user
	...
	PoThresholdCurrencyId             *int32           `bun:"po_threshold_currency_id,type:int"`                             // Currency ID for PO Approval Threshold
	...
	NetworkName                       *string          `bun:"network_name,type:varchar(30)"`                                 // Stores alternative login name
	...
	DefaultPickTicketPrinter          *string          `bun:"default_pick_ticket_printer,type:varchar(255)"`                 // Default printer for pick tickets in FCOE
	DefaultInvoicePrinter             *string          `bun:"default_invoice_printer,type:varchar(255)"`                     // Default printer for invoices in FCOE
	UserSignaturePath                 *string          `bun:"user_signature_path,type:varchar(255)"`
	...
	DfltUcc128LabelPrinter            *string          `bun:"dflt_ucc128_label_printer,type:varchar(255)"`               // Custom (F63490): the default printer for container (UCC128) labels for the shipping confirmation import
	AllowEdi855ManualOrdersFlag       *string          `bun:"allow_edi_855_manual_orders_flag,type:char(1)"`             // Setting to enable users for sending edi 855s for manual sales orders
	EmailSignature                    *string          `bun:"email_signature,type:varchar(max)"`                         // signature default for the user used in email
	...
	DefaultBssCustomerId              *float64         `bun:"default_bss_customer_id,type:decimal(19,0)"`                // Custom: Indicates the customer id to which OE should default when associated user’s OE defaults to a Builder Selection Sheet.
	...
	CheckCreationRole                 *int32           `bun:"check_creation_role,type:int"`                              // Role that describes the permission that the user has to interact with the
	...
	P21ClientLogPath                  *string          `bun:"p21_client_log_path,type:varchar(255)"`                     // Allows user to set a path for application error logging
	EnterpriseSearchUrl               *string          `bun:"enterprise_search_url,type:varchar(255)"`                   // URL path for Enterprise Search
