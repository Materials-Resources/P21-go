...
type ProdOrderHdr struct {
	...
	ExpectedCompletionDate   mssql.DateTime1  `bun:"expected_completion_date,type:datetime"`                        // Date the production order should be completed.
	...
	Comment                  *string          `bun:"comment,type:varchar(255)"`                                     // Production order comments.
	...
	ExpediteDate             *mssql.DateTime1 `bun:"expedite_date,type:datetime"`                            // The date in which this production order needs to be expedited in order to get assembly out on time
	EstimatedFreightCost     *float64         `bun:"estimated_freight_cost,type:decimal(19,9)"`              // Indicates estimated freight cost for the production order
	ActualFreightCost        *float64         `bun:"actual_freight_cost,type:decimal(19,9)"`                 // Indicates actual freight cost for the production order
	...
	ReleaseDate              *mssql.DateTime1 `bun:"release_date,type:datetime"`                             // The date when the order is released to production
	DrawingsComplete         *string          `bun:"drawings_complete,type:char(1),default:('N')"`           // Indicates that the drawings have been completed
	InventoryLocationId      *float64         `bun:"inventory_location_id,type:decimal(19,0)"`               // Default Inventory Location for production order
}
