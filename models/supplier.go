...
type Supplier struct {
	...
	BuyerId                         *string          `bun:"buyer_id,type:varchar(16)"`                                 // What buyer is associated with this supplier?
	...
	ControlValue                    *string          `bun:"control_value,type:varchar(16)"`                            // Weight/Volume/Dollars
	TargetValue                     *float64         `bun:"target_value,type:decimal(19,9)"`                           // Target amount (in Weight, Volume, Dollars, or Units) to purchase on a PO.
	ReviewCycle                     *float64         `bun:"review_cycle,type:decimal(3,0)"`                            //  How often (in days) a supplier’s line will be reviewed by a buyer.
	DateOfLastReview                *mssql.DateTime1 `bun:"date_of_last_review,type:datetime"`                         // When was the last time GPOR was ran for this supplier, defining requirements?
	DefaultCarrier                  *float64         `bun:"default_carrier,type:decimal(19,0)"`                        // Carrier most often used for shipments from this supplier
	DefaultShippingInstructions     *string          `bun:"default_shipping_instructions,type:varchar(255)"`           // Shipping instructions for this supplier.
	AverageLeadTime                 *float64         `bun:"average_lead_time,type:decimal(3,0)"`                       // What is the average lead time for this supplier
	LeadTimeSource                  *string          `bun:"lead_time_source,type:varchar(8)"`                          // Calculate lead time at the item or supplier level?
	LeadTimeSafetyFactor            *float64         `bun:"lead_time_safety_factor,type:decimal(19,4)"`                // Added to lead time to insure timely delivery.
	SafetyStockDays                 *float64         `bun:"safety_stock_days,type:decimal(19,4)"`                      // The number of extra days supply of material that should be maintained for the items purchased during this session.
	Fct                             *string          `bun:"fct,type:varchar(10)"`                                      // No longer used
	CurrencyId                      *float64         `bun:"currency_id,type:decimal(19,0)"`                            // What is the unique currency identifier for this supplier?
	SupplierName                    *string          `bun:"supplier_name,type:varchar(255)"`                           // What is the name of this supplier?
	...
	ShipDays                        *int32           `bun:"ship_days,type:int"`                                             // Number of days until receipt from the supplier ship date
	DeaCode                         *string          `bun:"dea_code,type:char(1)"`                                          // A special code to identify DEA
	DeaNumber                       *string          `bun:"dea_number,type:char(255)"`                                      // Indicates DEA Number
	DddReport                       *string          `bun:"ddd_report,type:char(1)"`                                        // YesNo value field to indicate DEA should appear on report
	LegacyId                        *string          `bun:"legacy_id,type:varchar(255)"`                                    // column to hold supplier ID from legacy app
	ParkerDivisionCd                *string          `bun:"parker_division_cd,type:varchar(10)"`                            // Item level custom column.
	ParkerSupplierFlag              *string          `bun:"parker_supplier_flag,type:char(1)"`                              // Y/N flag to determine if this is a supplier of Parker Hannifin goods.
	...
	PoApprovalThresholdFlag         *string          `bun:"po_approval_threshold_flag,type:char(1)"`                        // Flag indicating whether or not the PO approval threshold should be ignored for the current user on POs for this supplier
	SupplierSystemCd                *int32           `bun:"supplier_system_cd,type:int"`                                    // Custom: System used to communicate with this supplier
	PrintPoFlag                     *string          `bun:"print_po_flag,type:char(1)"`                                     // Indicates whether POs to this supplier will be printed by default. Supplier can have multiple default methods (print, fax, email, edi).
	FaxPoFlag                       *string          `bun:"fax_po_flag,type:char(1)"`                                       // Indicates whether POs to this supplier will be faxed by default. Supplier can have multiple default methods (print, fax, email, edi).
	EmailPoFlag                     *string          `bun:"email_po_flag,type:char(1)"`                                     // Indicates whether POs to this supplier will be emailed by default. Supplier can have multiple default methods (print, fax, email, edi).
	EdiPoFlag                       *string          `bun:"edi_po_flag,type:char(1)"`                                       // Indicates whether POs to this supplier will be sent via EDI by default. Supplier can have multiple default methods (print, fax, email, edi).
	...
	PallSupplierNumber              *string          `bun:"pall_supplier_number,type:varchar(20)"`                          // Holds the vendor Pall's supplier number. This is used for reporting information to the vendor Pall.
	...
	WeeklyTruckAllowedFlag          *string          `bun:"weekly_truck_allowed_flag,type:char(1)"`                         // Indicates that this supplier can make deliveries using weekly trucks
	RatelinxLocationId              *int32           `bun:"ratelinx_location_id,type:int"`                                  // Holds the associated location ID from the RateLinx system..
	DefaultPriceExpirationDate      *mssql.DateTime1 `bun:"default_price_expiration_date,type:datetime"`                    // Default price expiration date for all items for this supplier
	FreightFactor                   *float64         `bun:"freight_factor,type:decimal(19,9)"`                              // Percentage of item price that will be used to default incoming freight for this supplier.
	MinFreightFactor                *float64         `bun:"min_freight_factor,type:decimal(19,9)"`                          // Minimum incoming freight that should be defaulted (based on freight_factor) for this supplier.
	DealerWarrantyClaimsCd          *int32           `bun:"dealer_warranty_claims_cd,type:int"`                             // Determines if this supplier accepts dealer warranty claims.
	...
	ParkerAccountNo                 *string          `bun:"parker_account_no,type:varchar(255)"`                            // Parker account number.
	BrandNameCd                     *int16           `bun:"brand_name_cd,type:smallint"`                                    // Brand name code for the supplier, possible three values are 1399, 3059, 3060
	RcdSupplierFlag                 *string          `bun:"rcd_supplier_flag,type:char(1)"`                                 // Indicated whether this supplier is a RCD supplier
	SurchargeFlag                   string           `bun:"surcharge_flag,type:char(1),default:('N')"`                      // Indicates if Surcharge tab is enabled or not
	PoDefaultMethod                 *int32           `bun:"po_default_method,type:int"`                                     // Purchase Order Default Delivery Method
	...
	FidelitoneSupplierId            *string          `bun:"fidelitone_supplier_id,type:varchar(255)"`                       // Custom: Code used by Fidelitone to identify this supplier
	...
	DefaultPoPackingBasis           *string          `bun:"default_po_packing_basis,type:varchar(255)"`                     // Default packing basis to be used on purchase orders for this supplier.
	DfltSupplierRedemptionMethod    *int32           `bun:"dflt_supplier_redemption_method,type:int"`                       // Code representing the default supplier redemption method.
	SupplierRedemptionEmail         *string          `bun:"supplier_redemption_email,type:varchar(255)"`                    // Email address for supplier redemption.
	SupplierRedemptionFaxNumber     *string          `bun:"supplier_redemption_fax_number,type:varchar(20)"`                // Fax number for supplier redemption.
	ExportOrderInfoFlag             *string          `bun:"export_order_info_flag,type:char(1)"`                            // Y indicates customer order info is to be exported for POs
	MinimumProcessPoCost            *float64         `bun:"minimum_process_po_cost,type:decimal(19,9)"`                     // Minimum Process PO Cost
	BulkDiscountPercentage          *float64         `bun:"bulk_discount_percentage,type:decimal(19,9)"`                    // Custom: Discount percentage offered by supplier when qualified items are purchased in bulk.
	BulkContractNumberList          *string          `bun:"bulk_contract_number_list,type:varchar(255)"`                    // Custom: Comma delimited list of supplier contract numbers that qualify for a bulk discount.
	...
	CarrierShipMethodUid            *int32           `bun:"carrier_ship_method_uid,type:int"`                               // Carrier Ship Method Unique Identifier tied to the Carrier Ship Method UId from the carrier_ship_method table
	DoNotCreateUnapprovedPoFlag     *string          `bun:"do_not_create_unapproved_po_flag,type:char(1)"`                  // Custom column to indicate not to create unapproved po for 'RO and ERO' sales order priority backorders from oe.
