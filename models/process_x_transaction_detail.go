...
type ProcessXTransactionDetail struct {
	...
	StartDate              *mssql.DateTime1 `bun:"start_date,type:datetime"`                                      // Date the stage was started.
	...
	CostUom                *string          `bun:"cost_uom,type:varchar(8)"`                                      // Describes the unit of measure of the cost of the stage. (ex. $10 per BOX)
	StageQtyUom            *string          `bun:"stage_qty_uom,type:varchar(8)"`                                 // Describes the unit of measure the material gets processed in. (Ex. Feet -  Pounds)
	...
	CostType               *int16           `bun:"cost_type,type:smallint"`                                       // Indicates whether the cost is determined by Weight of the raw item or by Unit of Measure for the raw item.
	...
	StageWipAccountNumber  *string          `bun:"stage_wip_account_number,type:varchar(32)"`                     // The GL account affected by the costs associated with a stage while work is in progress.
	StageCd                *string          `bun:"stage_cd,type:varchar(255)"`                                    // Code used to identify the stage.
	StageDesc              *string          `bun:"stage_desc,type:varchar(8000)"`                                 // A brief description of the stage.
	EstimatedHours         *float64         `bun:"estimated_hours,type:decimal(6,2)"`                             // Estimated hours it should take to complete the stage.
	SupplierId             *float64         `bun:"supplier_id,type:decimal(19,0)"`                                // Supplier for stages that require a po.
	VendorId               *float64         `bun:"vendor_id,type:decimal(19,0)"`                                  // Vendor for stages that require a po.
	MinPoCost              *float64         `bun:"min_po_cost,type:decimal(19,9)"`                                // Minimum po cost for stages that require a po.
	BuyerId                *string          `bun:"buyer_id,type:varchar(16)"`                                     // Buyer for stages that require a po.
	DivisionId             *float64         `bun:"division_id,type:decimal(19,0)"`                                // Division for stages that require a po.
	StagePoDesc            *string          `bun:"stage_po_desc,type:varchar(255)"`                               // Description to print on pos for stages that require a po.
	PurchasingWeight       *float64         `bun:"purchasing_weight,type:decimal(19,9)"`                          // store purchasing weight value for the stage
	WipInvMastUid          *int32           `bun:"wip_inv_mast_uid,type:int"`                                     // Column holds inv_mast_uid of the item at the point that a process is performed on the item.
	BackflushFlag          *string          `bun:"backflush_flag,type:char(1)"`                                   // Determines whether the process will allow Backflushing functionality
	...
	ConsolidatableFlag     *string          `bun:"consolidatable_flag,type:char(1)"`                              // Indicates whether this process (stage) is eligible to be combined on a consolidated Process PO with processes from other process Transaction.
	ContainerInvMastUid    *int32           `bun:"container_inv_mast_uid,type:int"`                               // Key to item table for fillable container item ID
	ContainerUom           *string          `bun:"container_uom,type:varchar(8)"`                                 // UOM to use for creating adjustment for fillable container use
	ContainerFlag          *string          `bun:"container_flag,type:char(1)"`                                   // Flag indicating this process step has a fillable container
	CommentProcess         string           `bun:"comment_process,type:char(1),default:('N')"`                    // indicate if this process is comment process or not
	Comment                *string          `bun:"comment,type:varchar(max)"`                                     // let user enter comment
}
