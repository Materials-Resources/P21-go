...
type BusinessRule struct {
	...
	FieldName                  *string         `bun:"field_name,type:varchar(255)"`                                 // The field for which the rule applies
	ClassName                  *string         `bun:"class_name,type:varchar(255)"`                                 // The class for which the rule applies
	...
	WindowName                 *string         `bun:"window_name,type:varchar(255)"`                                // Name of the window containing the rule
	WindowTitle                *string         `bun:"window_title,type:varchar(255)"`                               // Title of the window containing the rule
	LicenseKey                 *string         `bun:"license_key,type:varchar(255)"`                                // Key for enabling this rule if DC Rules is not licensed
	...
	BusinessRuleEventUid       *int32          `bun:"business_rule_event_uid,type:int"`                             // Determines the event from which this rule will be triggered.
	...
	BusinessRuleEventKeyUid    *int32          `bun:"business_rule_event_key_uid,type:int"`                         // UID to event/key record that determines the specific instance of an event that this rule applies to.
	...
	RulePageUrl                *string         `bun:"rule_page_url,type:varchar(8000)"`                             // URL for the ASP.Net page to be launched by a web visual business rule.
	...
	OrderEntryFlag             *string         `bun:"order_entry_flag,type:char(1)"`                                // Indicate if the rule for Order Entry window be executed or not
	FrontCounterFlag           *string         `bun:"front_counter_flag,type:char(1)"`                              // Indicate if the rule for Front Counter window be executed or not
	RmaEntryFlag               *string         `bun:"rma_entry_flag,type:char(1)"`                                  // Indicate if the rule for RMA Entry window be executed or not
}
