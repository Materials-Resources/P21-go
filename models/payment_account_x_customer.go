...
type PaymentAccountXCustomer struct {
	...
	CreditcardProcessorUid     *int32          `bun:"creditcard_processor_uid,type:int"`                            // Associates, where necessary, the Customer Payment Account with an electronic payment processor record.
	AutomaticPaymentFlag       *string         `bun:"automatic_payment_flag,type:char(1)"`                          // Column flag to indicates an automatic payment process.
	AutomaticPaymentLimit      *float64        `bun:"automatic_payment_limit,type:decimal(19,9)"`                   // Indicates the limit amount when corresponding flag is selected.
	EpfMerchantAccountUid      *int32          `bun:"epf_merchant_account_uid,type:int"`                            // Associates, where necessary, the Customer Payment Account with an EPF merchant account record.
}
