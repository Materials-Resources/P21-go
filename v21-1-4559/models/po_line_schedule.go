...
type PoLineSchedule struct {
	...
	ReleaseDate       mssql.DateTime1 `bun:"release_date,type:datetime"`           // Date this release should be received.
	...
	RevisionCode      *int32          `bun:"revision_code,type:int"`               // What has happened to this release?  Date changed, Qty Changed?
	ExpediteFlag      *string         `bun:"expedite_flag,type:char(1)"`           // Indicates whether this schedule line should be included on the expedite request.
	OeLineScheduleUid *int32          `bun:"oe_line_schedule_uid,type:int"`        // Unique Identifier of oe_line_schedule table
	QtyReady          *float64        `bun:"qty_ready,type:decimal(19,9)"`         // Quantity ready for container building
}
