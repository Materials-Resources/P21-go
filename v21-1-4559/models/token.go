...
type Token struct {
	...
	DataTypeCd       *int32          `bun:"data_type_cd,type:int"`                                     // Store a code that will let the program know how to build a where clause.
	AvailableAreas   *int32          `bun:"available_areas,type:int,unique"`                           // Available areas in the system for this token
	CodeGroupNo      *int16          `bun:"code_group_no,type:smallint"`                               // System assigned code for the code group that relates to this token.
	UserLookupFlag   *string         `bun:"user_lookup_flag,type:char(1)"`                             // Indicates that the token represents a field that will be used as part of a query to find other associated data.
}
