...
type OeHdr struct {
	...
	OrderDate                   *mssql.DateTime1 `bun:"order_date,type:datetime"`                        // The date the order was taken.
	...
	RequestedDate               *mssql.DateTime1 `bun:"requested_date,type:datetime"`                    // The date by which the entire order is needed.
	...
	RmaExpirationDate           *mssql.DateTime1 `bun:"rma_expiration_date,type:datetime,default:(getdate())"` // Date before which customer must return all material designated on this RMA.
	...
	TagHoldCancelDate           *mssql.DateTime1 `bun:"tag_hold_cancel_date,type:datetime"`                        // Tag and Hold Cancel Date
	...
	DateOrderCompleted          *mssql.DateTime1 `bun:"date_order_completed,type:datetime"`                        // The date an order is marked complete Y
	...
	AcknowledgementDate         *mssql.DateTime1 `bun:"acknowledgement_date,type:datetime"`                        // According to the spec for Feature 22107, the aknowledgement date is a promised date
	...
	RequestedShipDate           *mssql.DateTime1 `bun:"requested_ship_date,type:datetime"`                         // Custom (F23038): date that this order is requested to ship
	ExpectedCompletionDate      *mssql.DateTime1 `bun:"expected_completion_date,type:datetime"`                    // Date service order is expected to be completed
	...
	PmDate                      *mssql.DateTime1 `bun:"pm_date,type:datetime"`                                     // Date that the preventative maintenace for the service items on the generated service order should take place.
	...
	*/
	...
	RoutedEtaDate                  *mssql.DateTime1 `bun:"routed_eta_date,type:datetime"`                      // ETA of an order that has already been put through geocom routing.
	RouteOverrideDate              *mssql.DateTime1 `bun:"route_override_date,type:datetime"`                  // Data on the order header used during a geocom export
	ExpediteDate                   *mssql.DateTime1 `bun:"expedite_date,type:datetime"`                        // Default item expedite date
	OrderOpenStartDate             *mssql.DateTime1 `bun:"order_open_start_date,type:datetime"`                // Custom column to store the date when saving an order as marked with validation status COD or Hold.
	...
	PromiseDateEditedDate          *mssql.DateTime1 `bun:"promise_date_edited_date,type:datetime"`             // Date a promise date was edited
	...
	UserDefinedDate                *mssql.DateTime1 `bun:"user_defined_date,type:datetime"`                      // A date field that the user defines on the order.
	...
	SecondRouteOverrideDate        *mssql.DateTime1 `bun:"second_route_override_date,type:datetime"`    // Second Geocom Override Date
	...
	DocumentCaptureDate            *mssql.DateTime1 `bun:"document_capture_date,type:datetime"`                    // Time of capture to document management system, used to detect order ack and quotation edits
	...
	PrebillingDate                 *mssql.DateTime1 `bun:"prebilling_date,type:datetime"`                          // Prebilling Date
	...
	SlaRequestedDate               *mssql.DateTime1 `bun:"sla_requested_date,type:datetime"`                      // Custom: Original required date calculated using the ship-to or customer's Service Level Agreement.
	...
	RentalReturnDate               *mssql.DateTime1 `bun:"rental_return_date,type:datetime"`                      // Defines the time the order items will be rented
	...
	RentalStartDate                *mssql.DateTime1 `bun:"rental_start_date,type:datetime"`                       // Start Date for a rental order
}
