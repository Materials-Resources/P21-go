...
type Lot struct {
	...
	CreationDate                mssql.DateTime1  `bun:"creation_date,type:datetime,default:(getdate())"`               // Date lot was created.
	...
	CertificationDate           *mssql.DateTime1 `bun:"certification_date,type:datetime"`                       // lot certification date
	SupplierCertificationDate   *mssql.DateTime1 `bun:"supplier_certification_date,type:datetime"`              // supplier certification date
	RecertificationDate         *mssql.DateTime1 `bun:"recertification_date,type:datetime"`                     // re-certification date
	SupplierRecertificationDate *mssql.DateTime1 `bun:"supplier_recertification_date,type:datetime"`            // supplier re-certification date
	QualityCdUid                *int32           `bun:"quality_cd_uid,type:int"`                                // Uniquely identify the quality code
	ExpirationDate              *mssql.DateTime1 `bun:"expiration_date,type:datetime"`                          // indicate lot expiration date
	ShelfLifeOriginDate         *mssql.DateTime1 `bun:"shelf_life_origin_date,type:datetime"`                   // Actual date shelf life began
	ShelfLifeExpDate            *mssql.DateTime1 `bun:"shelf_life_exp_date,type:datetime"`                      // Shelf life expiration date: the date the product becomes unsuitable to sell
	ShelfLifeCalcExpDate        *mssql.DateTime1 `bun:"shelf_life_calc_exp_date,type:datetime"`                 // Shelf life calculated expiration date: expiration date calculated from origin date and supplier/item settings.
