...
type SalesrepCommission struct {
	...
	SalesrepId                *string         `bun:"salesrep_id,type:varchar(16),unique"`                        // What sales representative does this relationship refer to?
	...
	NumberOfDaysOverdue       *float64        `bun:"number_of_days_overdue,type:decimal(3,0)"`                   // Ddetermines how many days must pass after an invoice is overdue before commission is not paid.
	TrackQuotas               string          `bun:"track_quotas,type:char(1)"`                                  // Indicates if quotas are tracked for this salesrep.
	CommissionScheduleId      *string         `bun:"commission_schedule_id,type:varchar(8)"`                     // User-defined code that identifies a commission schedule.
	...
	TerritoryUid              *int32          `bun:"territory_uid,type:int,unique"`                              // Custom This column will hold the territory used for commission calcs.
	HouseSplitPercentage      *float64        `bun:"house_split_percentage,type:decimal(19,9)"`                  // Decimal value indicating a split percentage that goes to the house when commissions are calculated for the salesrep.
