...
type RmaReceiptLine struct {
	...
	ReasonAdjustmentId *string   `bun:"reason_adjustment_id,type:varchar(5)"`                         // Reason ID associated w/qty_to_adjust.
	...
	UnitOfMeasure      *string   `bun:"unit_of_measure,type:varchar(8)"`                              // Unit of measure chosen to describe the measurement of qtys on this receipt line.
	UnitSize           *float64  `bun:"unit_size,type:decimal(19,9)"`                                 // The number of SKUs associated w/the unit of measure defined for this receipt line.
	QtyToTransfer      *float64  `bun:"qty_to_transfer,type:decimal(19,9)"`                           // Custom column to account amount of receipt qty to transfer to the destination location
	CoreItemCost       *float64  `bun:"core_item_cost,type:decimal(19,9)"`                            // Custom column for core item supplier cost.
	CountryOfOrigin    *string   `bun:"country_of_origin,type:varchar(255)"`                          // Custom column to store country of origin for item in RMA receipts.
}
