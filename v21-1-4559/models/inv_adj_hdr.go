...
type InvAdjHdr struct {
	...
	InvAdjDescription     *string         `bun:"inv_adj_description,type:varchar(255)"`                     // Text description of the cause for the adjustment.
	ParentTransNo         *int32          `bun:"parent_trans_no,type:int"`                                  // Source of the adjustment, decode from code_p21.
	ParentTransTypeCd     *int32          `bun:"parent_trans_type_cd,type:int"`                             // If a document is associated with an adjustment, the doc number may be stored here.
	RfLastLineNo          *int32          `bun:"rf_last_line_no,type:int"`                                  // Corresponds to last line number counted on inv_adj_line (for saving place on RF Counting).
	...
	SortOrder             *string         `bun:"sort_order,type:char(1)"`                                   // Determines whether user was grouping by Bin or Item in Physical Count.
	AffectActualUsageFlag *string         `bun:"affect_actual_usage_flag,type:char(1)"`                     // Affect Actual Usage Flag
