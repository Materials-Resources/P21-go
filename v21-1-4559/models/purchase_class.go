...
type PurchaseClass struct {
	...
	HighLevelMonths             *float64  `bun:"high_level_months,type:decimal(19,2)"`                      // Order up to levels in terms of month’s supply for high velocity items.
	MidLevelMonths              *float64  `bun:"mid_level_months,type:decimal(19,2)"`                       // Order up to levels in terms of month’s supply for medium velocity items.
	LowLevelMonths              *float64  `bun:"low_level_months,type:decimal(19,2)"`                       // Order up to levels in terms of month’s supply for low velocity items.
	AnnualThreshold             *float64  `bun:"annual_threshold,type:decimal(19,4)"`                       // Annual cost used for ranking purposes.
	HighSafetyStockFactor       *float64  `bun:"high_safety_stock_factor,type:decimal(19,4)"`               // Safety stock factor (applied against the safety days) for high volume items.
	...
	ErraticMaxFactor            *float64  `bun:"erratic_max_factor,type:decimal(19,4)"`                     // Used in min/max identification.  Multiplied against the new min for erratic items to determine the new max.
	NonerraticMinFactor         *float64  `bun:"nonerratic_min_factor,type:decimal(19,4)"`                  // Used in min/max identification.  Multiplied against the purchasing unit size for nonerratic items to suggest a new min.
	NonerraticMaxFactor         *float64  `bun:"nonerratic_max_factor,type:decimal(19,4)"`                  // Used in min/max identification.  Multiplied against the new min for nonerratic items to determine the new max.
	IncludeLargeOrderQtyFlag    *string   `bun:"include_large_order_qty_flag,type:char(1)"`                 // Custom F42518: determines if item is assigned this purchase class are included in the OE unusually large order qty functionality
