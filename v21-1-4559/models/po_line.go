...
type PoLine struct {
	...
	ReceivedDate             *mssql.DateTime1 `bun:"received_date,type:datetime"`                                   // Date the item was last received for this PO.
	...
	DateDue                  *mssql.DateTime1 `bun:"date_due,type:datetime"`                                        // When is this item due from the supplier?
	...
	PricingBookEffectiveDate *mssql.DateTime1 `bun:"pricing_book_effective_date,type:datetime"`                     // Date on which the pricing book takes effect.
	...
	RequiredDate             *mssql.DateTime1 `bun:"required_date,type:datetime"`                                   // When is this order required by?
	...
	SupplierShipDate         *mssql.DateTime1 `bun:"supplier_ship_date,type:datetime"`                  // Date supplier will ship the item
	...
	AcknowledgedDate         *mssql.DateTime1 `bun:"acknowledged_date,type:datetime"`                   // Date the supplier has acknowledged the fact that the PO needs to shipped to meet a required date
	...
	DateDueLastModified      *mssql.DateTime1 `bun:"date_due_last_modified,type:datetime"`
