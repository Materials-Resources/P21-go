...
type CycleCountDetail struct {
	...
	Bin                      *string         `bun:"bin,type:varchar(10)"`                             // The bin counted
	InvMastUid               int32           `bun:"inv_mast_uid,type:int"`                            // Unique identifier for the item id.
	QtyCounted               *float64        `bun:"qty_counted,type:decimal(19,9)"`                   // The quantity counted.
	...
	QtyOnHandAtPhysicalCount *float64        `bun:"qty_on_hand_at_physical_count,type:decimal(19,9)"` // Qty on hand for items not counted at physical count
}
