...
type Delivery struct {
	...
	BeginShipmentDate        *mssql.DateTime1 `bun:"begin_shipment_date,type:datetime"`                       // Date Time that the delivery started. Time that the driver clocked in at the first stop.
	EndShipmentDate          *mssql.DateTime1 `bun:"end_shipment_date,type:datetime"`                         // Date Time that the delivery ended. Time that the driver clocked out at the last stop.
	DriverId                 string           `bun:"driver_id,type:varchar(16)"`                              // ID of the driver, from the contacts table.
	DownloadStatus           *int32           `bun:"download_status,type:int"`                                // Not downloaded, downloaded or uploaded.
	RowStatusFlag            int16            `bun:"row_status_flag,type:smallint"`                           // Indicates current record status.
	DeliveryListDate         mssql.DateTime1  `bun:"delivery_list_date,type:datetime"`                        // Date the the list was last edited.
	...
	GpsData                  *string          `bun:"gps_data,type:text"`                                      // GPS Tracking data of the delivery
