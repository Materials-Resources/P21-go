...
type Stage struct {
	...
	MinimumPoCost         *float64  `bun:"minimum_po_cost,type:decimal(19,9)"`                            // What is the minimum cost allowed before a Stage Purchase order is created?
	RowStatusFlag         int16     `bun:"row_status_flag,type:smallint"`                                 // Indicates current record status.
	BuyerId               *string   `bun:"buyer_id,type:varchar(16)"`                                     // What buy is associated with this stage?
	...
	LocationId            *float64  `bun:"location_id,type:decimal(19,0)"`                                // Location for the stage
	StagePoDesc           *string   `bun:"stage_po_desc,type:varchar(255)"`                               // Provides a description of a stage for a specific process. Only applicable if stage.stage_po = Y.
