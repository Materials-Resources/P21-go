...
type OeLineSchedule struct {
	...
	ReleaseDate             mssql.DateTime1  `bun:"release_date,type:datetime"`                                    // Release date
	...
	PickDate                *mssql.DateTime1 `bun:"pick_date,type:datetime"`                                       // Date that items on the release schedule should be picked.
	Disposition             *string          `bun:"disposition,type:char(1)"`                                      // Disposition of the open quantity.
	ExpediteDate            *mssql.DateTime1 `bun:"expedite_date,type:datetime"`                                   // Date that the system will allocate the items to an order
	...
	OriginalPromiseDate     *mssql.DateTime1 `bun:"original_promise_date,type:datetime"`                           // The original promise date.
	PromiseDate             *mssql.DateTime1 `bun:"promise_date,type:datetime"`                                    // The current promise date.
	PromiseDateEditedDate   *mssql.DateTime1 `bun:"promise_date_edited_date,type:datetime"`                        // The date that the promise date was edited.
