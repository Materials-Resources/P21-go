...
type FreightCode struct {
	...
	FreeFreightBasisCd            *int32     `bun:"free_freight_basis_cd,type:int"`                               // What is free freight based on (order or shipment)
	FreeInFreightMin              *float64   `bun:"free_in_freight_min,type:decimal(19,9)"`                       // Incoming minimum for free freight
	FreeOutFreightMin             *float64   `bun:"free_out_freight_min,type:decimal(19,9)"`                      // Outgoing minimum for free freight
	DirectShipFreeFreightFlag     *string    `bun:"direct_ship_free_freight_flag,type:char(1)"`                   // Allow free freight on Direct Shipments flag
	FreeInFreightMinWeb           *float64   `bun:"free_in_freight_min_web,type:decimal(19,9)"`                   // Incoming minimum for free freight via web orders
	FreeOutFreightMinWeb          *float64   `bun:"free_out_freight_min_web,type:decimal(19,9)"`                  // Outgoing minimum for free freight via web orders
	HandlingChargeOptionCd        *int32     `bun:"handling_charge_option_cd,type:int"`                           // The value indicates how to process freight and handling charges for direct shipping
	ExternalTaxProductCodeIn      *string    `bun:"external_tax_product_code_in,type:varchar(255)"`               // The product code to be used for 3rd party tax systems on incoming freight
	ExternalTaxProductCodeOut     *string    `bun:"external_tax_product_code_out,type:varchar(255)"`              // The product code to be used for 3rd party tax systems on outgoing freight
	...
	PaySpecialFlag                *string    `bun:"pay_special_flag,type:char(1)"`                                // For the UPS ConnectShip integration, indicates that freight will be charged for 'M' class items only when the freight minimum is met
	SkipFirstShipmentFlag         *string    `bun:"skip_first_shipment_flag,type:char(1)"`                        // For the UPS ConnectShip integration, indicates that freight will not be paid for special items (as determiend by the item class) for the first shipment of each month
	...
	DeductibleFlag                *string    `bun:"deductible_flag,type:char(1)"`                                 // Indicates whether this freight code is deductible type (Custom Feature)
	...
	FedexPaymentMethod            *int32     `bun:"fedex_payment_method,type:int"`                                // Fedex payment method
