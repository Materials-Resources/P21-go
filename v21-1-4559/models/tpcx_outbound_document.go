...
type TpcxOutboundDocument struct {
	...
	ApplicationServerLocation *string   `bun:"application_server_location,type:varchar(255)"`                 // Jaguar server
	...
	SendmessageReturnCode     *int16    `bun:"sendmessage_return_code,type:smallint"`                         // Return code from the QueueMessage component
	ErrorMessage              *string   `bun:"error_message,type:varchar(255)"`                               // Any information about why the message could not be transmitted.
	...
	Document                  *string   `bun:"document,type:text"`                                            // XML document being sent (IMAGE NOT TEXT)
}
