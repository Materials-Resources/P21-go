...
type CashDrawerTransaction struct {
	...
	TransactionAcctNo        *string   `bun:"transaction_acct_no,type:varchar(32)"`                         // Transaction account number - Account when making a withdrawal
	TransactionTypeCd        int32     `bun:"transaction_type_cd,type:int"`                                 // Remittance / Deposit / Withdrawal.
	CcUserId                 *string   `bun:"cc_user_id,type:varchar(30)"`                                  // User responsible for this transaction.
	DateCreated              time.Time `bun:"date_created,type:datetime,default:(getdate())"`               // Date and time the record was originally created
	CreatedBy                string    `bun:"created_by,type:varchar(255),default:(suser_sname())"`         // User who created the record
	DateLastModified         time.Time `bun:"date_last_modified,type:datetime,default:(getdate())"`         // Date and time the record was modified
	LastMaintainedBy         string    `bun:"last_maintained_by,type:varchar(255),default:(suser_sname())"` // User who last changed the record
	InvoiceNo                *string   `bun:"invoice_no,type:varchar(10)"`                                  // Invoice Number for transactions not tied to a remittance/order
	PaymentNumber            *float64  `bun:"payment_number,type:decimal(19,0)"`                            // Associated Payment NUmber
}
