...
type PopupDetail struct {
	...
	OnStartLoad                         *bool           `bun:"on_start_load,type:bit"`                                         // Column to determine if the popup should load when its opened
	AutoFiltering                       *bool           `bun:"auto_filtering,type:bit"`                                        // Column to determine if the partial key should filter as the user types in
	FocusOnGrid                         *bool           `bun:"focus_on_grid,type:bit"`                                         // Column to determine if the popup grid should be focused when the popup is opened
	ColumnsResizable                    *bool           `bun:"columns_resizable,type:bit"`                                     // Column to determine if the popup grid columns can be resized
	ExportEnabled                       *bool           `bun:"export_enabled,type:bit"`                                        // If the popup's grid can be exported
	GroupingEnabled                     *bool           `bun:"grouping_enabled,type:bit"`                                      // If the popup's grid can be grouped
	ColumnFilteringEnabled              *bool           `bun:"column_filtering_enabled,type:bit"`                              // If the columns filters are enabled
	CountRetrievalEnabled               *bool           `bun:"count_retrieval_enabled,type:bit"`                               // If the count of rows should be retrieved
	GenericSearchEnabled                *bool           `bun:"generic_search_enabled,type:bit"`                                // If the generic search feature is enabled
	MultipleRowSelectionEnabled         *bool           `bun:"multiple_row_selection_enabled,type:bit"`                        // If multiple rows can be selected.
	UserConfigurationEnabled            *bool           `bun:"user_configuration_enabled,type:bit"`                            // If the popup's user configuration is enabled
	CoreFlag                            *bool           `bun:"core_flag,type:bit"`                                             // If the popup is a core popup
	DeleteFlag                          *bool           `bun:"delete_flag,type:bit"`                                           // If the popup is deleted
	InactiveFlag                        *bool           `bun:"inactive_flag,type:bit"`                                         // If the popup is inactive
	AdditionalWhereEnabled              *bool           `bun:"additional_where_enabled,type:bit"`                              // Determines if the popup should apply additional conditions to its where
	PopupAttribute                      *int32          `bun:"popup_attribute,type:int"`                                       // The attibute that defines the popup
	RankingEnabled                      *bool           `bun:"ranking_enabled,type:bit"`                                       // Determines if the ranking feature is enabled
	Source                              *string         `bun:"source,type:varchar(500)"`                                       // Place where the definition of the popup has been extracted
	ExpandablePopup                     *string         `bun:"expandable_popup,type:varchar(50)"`                              // Sets the column name of the popup to use in an expandable popup.
	CompSecEnabled                      *bool           `bun:"comp_sec_enabled,type:bit"`                                      // Determines if the popup should apply company security its query
