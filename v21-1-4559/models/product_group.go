...
type ProductGroup struct {
	...
	RevenueAccountNo             *string          `bun:"revenue_account_no,type:varchar(80)"`                           // The default revenue account number for this product group.
	CosAccountNo                 *string          `bun:"cos_account_no,type:varchar(80)"`                               // Default COS account for this product group.
	...
	ParkerProductCode            *string          `bun:"parker_product_code,type:varchar(255)"`                         // stores the parker product code for this group.
	TaxGroupId                   *string          `bun:"tax_group_id,type:varchar(10)"`                                 // Foreign key to the table tax_group_hdr
	CompletionLeadTime           *int32           `bun:"completion_lead_time,type:int"`                                 // This custom column will store the completion lead days needed for this product group
	RequiredLeadTime             *int32           `bun:"required_lead_time,type:int"`                                   // This custom column will store the required lead days needed for this product group
	ProductGroupUid              int32            `bun:"product_group_uid,type:int,autoincrement,unique,identity"`      // Unique internal ID number for this table.
	...
	RmaRevenueAccountNo          *string          `bun:"rma_revenue_account_no,type:varchar(80)"`                       // RMA Revenue Account
	...
	PriceRoundingThreshold       *float64         `bun:"price_rounding_threshold,type:decimal(19,9)"`                   // The price amount above which to apply price rounding.
	...
	MinimumOrderLineProfit       *float64         `bun:"minimum_order_line_profit,type:decimal(19,9)"`                  // Minimum profit
	MaximumOrderLineProfit       *float64         `bun:"maximum_order_line_profit,type:decimal(19,9)"`                  // Maximum profit
	...
	CardlockProductGroupTypeCd   *int32           `bun:"cardlock_product_group_type_cd,type:int"`                       // The type of cardlock product type of a product group
	TaRentalRevenueAccountNo     *string          `bun:"ta_rental_revenue_account_no,type:varchar(32)"`                 // TrackAbout Rental Revenue Account for product group
	PercentReqToAllocTransfer    *float64         `bun:"percent_req_to_alloc_transfer,type:decimal(19,9)"`              // Custom: Indicates that items with this corresponding product group will only be allocated to transfers when this percent of the quantity is available.
	FutureEffectiveDate          *mssql.DateTime1 `bun:"future_effective_date,type:datetime"`                           // Has the date from where the price rounding threshold can be updated.
	FuturePriceRoundingThreshold *float64         `bun:"future_price_rounding_threshold,type:decimal(19,9)"`            // Contains the new price threshold that is going to be updated into
}
