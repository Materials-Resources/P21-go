...
type InventoryReturnHdr struct {
	...
	ReturnDate                  mssql.DateTime1 `bun:"return_date,type:datetime,default:(getdate())"`                // Date inventory return was entered
	ExpectedDate                mssql.DateTime1 `bun:"expected_date,type:datetime,default:(getdate())"`              // What is the expected date that this relatinship between a process and a transaction should be completed?
	...
	RmaNumber                   *string         `bun:"rma_number,type:varchar(255)"`                                 // Used by some users to track vendors authorization number.
	PoReferenceNumber           *float64        `bun:"po_reference_number,type:decimal(19,0)"`                       // PO Number for the items being returned
	Ship2Name                   string          `bun:"ship2_name,type:varchar(255)"`                                 // Ship to name.
	Ship2Address1               *string         `bun:"ship2_address1,type:varchar(255)"`                             // What is the first line of the ship to address?
	Ship2Address2               *string         `bun:"ship2_address2,type:varchar(255)"`                             // What is the second  line of the ship to address?
	Ship2City                   *string         `bun:"ship2_city,type:varchar(255)"`                                 // What city should this order be shipped to?
	Ship2State                  *string         `bun:"ship2_state,type:varchar(255)"`                                // What is the state  for the ship to address?
	Ship2Zip                    *string         `bun:"ship2_zip,type:varchar(255)"`                                  // What postal code should this order be shipped to?
	Ship2Country                *string         `bun:"ship2_country,type:varchar(255)"`                              // What country should this order be shipped to?
	...
	Ship2Phone                  *string         `bun:"ship2_phone,type:varchar(20)"`                                 // Ship to phone number
	CarrierId                   *float64        `bun:"carrier_id,type:decimal(19,0)"`                                // What is the id of this carrier (if any)?
	CarrierTrackingNo           *string         `bun:"carrier_tracking_no,type:varchar(255)"`                        // The tracking number assigned by the carrier
	ChargeFreightToVendor       string          `bun:"charge_freight_to_vendor,type:char(1),default:('Y')"`
	ContactId                   *string         `bun:"contact_id,type:varchar(16)"`                 // Contact associated with the inventory return
	RetrievedByWms              string          `bun:"retrieved_by_wms,type:char(1),default:('N')"` // Determines whether or not this record has been exported.
	CurrencyLineUid             *int32          `bun:"currency_line_uid,type:int"`                  // Used to identify currency info for the Inventory Return, namely the exchange rate for a foreign currency transaction.
	...
	Ship2Address3               *string         `bun:"ship2_address3,type:varchar(255)"`            // Ship to address line 3
	BlindShipFlag               *string         `bun:"blind_ship_flag,type:char(1)"`                // Custom column to set an inventory return as a blind shipment so it does not appear to come from the distributor.
	Fob                         *string         `bun:"fob,type:varchar(20)"`                        // custom column to default from division maint
	TrackingNoList              *string         `bun:"tracking_no_list,type:varchar(1000)"`         // Custom column to hold multiple comma-delimited tracking no list
	ExpediteNotes               *string         `bun:"expedite_notes,type:varchar(8000)"`           // Notes which help expedite inventory returns.
	WarrantyInventoryReturnFlag *string         `bun:"warranty_inventory_return_flag,type:char(1)"` // Custom column to indicate the inventory return is a warranty inventory return
}
