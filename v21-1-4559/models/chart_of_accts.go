...
type ChartOfAccts struct {
	...
	MaintainEncumbrances       *string         `bun:"maintain_encumbrances,type:char(1)"`                        // Does this account maintain encumbrances?
	CreatedBy                  *string         `bun:"created_by,type:varchar(255),default:(suser_sname())"`
	AccountInformation         *string         `bun:"account_information,type:varchar(255)"`                     // a field for extended information about the account
	...
	RecordTypeCd               *int32          `bun:"record_type_cd,type:int"`                                   // Determine the type of record (User Defined, System Defined, etc...)
	DocLinkSmartFormFlag       *string         `bun:"doc_link_smart_form_flag,type:char(1)"`                     // Flag to indicate that the associated gl account is valid to use with doc-link smart forms.
	AccountDefinitionCd        *int32          `bun:"account_definition_cd,type:int"`                            // Determines whether the Account is an AR/AP Account
}
