...
type OeSchedule struct {
	...
	FirstDate         *mssql.DateTime1 `bun:"first_date,type:datetime"`                                      // First release date for Rule type schedules.
	TotalReleases     *float64         `bun:"total_releases,type:decimal(19,0)"`                             // Total number of releases for Rule type schedules.
	FrequencyValue    *float64         `bun:"frequency_value,type:decimal(19,0)"`                            // Time in terms of frequency_type between releases for Rule type schedules
	FrequencyType     *string          `bun:"frequency_type,type:varchar(8)"`                                // Unit to determine frequency_value for Rule type schedules (Day, Month, etc).
	ExpediteValue     *float64         `bun:"expedite_value,type:decimal(19,0)"`                             // Time in terms of expedite_type to expedite individual schedules.
	ExpediteType      *string          `bun:"expedite_type,type:varchar(8)"`                                 // Unit to determine expedite_value (Day, Month, etc).
	PickValue         *float64         `bun:"pick_value,type:decimal(19,0)"`                                 // Time in terms of pick_type to pick individual schedules.
	...
	ApplyToLineMethod *int32           `bun:"apply_to_line_method,type:int"`                                 // Custom - Which method should be used to apply this schedule to line items?
}
